...
class TUH_Dataset_OLD(Dataset):
    def __init__(self, root_path, data_path, csv_path, size=None,
        ...
        # Path for saving/loading patient splits with seed
        self.splits_path = os.path.join(self.root_path, f"tuh_patient_splits.pkl")
    ...
    def __read_data__(self):
        ...
        try:
            ...
            raise ValueError(f"Split file {self.splits_path} not found. Please create splits first.")
    ...
    def __getitem__(self, index):
        ...
        # filename as patient_id + id (3 digists)
        filename = self.selected_files[file_index].split('_preprocessed.npy')[0] + f'_id_{local_index:03d}'
        # return seq_x, seq_y
        return {
                "past_values": seq_x.copy(), 
...
def get_tuh_dataloaders_old_splits(root_path, data_path, csv_path, batch_size=64, num_workers=4, 
                   prefetch_factor=1, pin_memory=False, drop_last=False, size=None, seed=42):
    # Create datasets
    train_dataset = TUH_Dataset_OLD(root_path, data_path, csv_path, size=size, split='train', seed=seed)
    val_dataset = TUH_Dataset_OLD(root_path, data_path, csv_path, size=size, split='val', seed=seed)
    test_dataset = TUH_Dataset_OLD(root_path, data_path, csv_path, size=size, split='test', seed=seed)
