...
def read_mef_file(file_path):
    """Read an EDF file and return a raw MNE object."""
    try:
        logger.debug(f"Reading MEF file: {file_path}")
        raw = mef_to_mne(file_path)
        logger.debug(f"Successfully read MEF file: {file_path}")
        return raw
...
def select_channels(raw, channels_to_use_ref):
    """
    ...
    #     channels_to_use_ref = channel_lists['mayo_reduced']
    ...
    available_channels = [ch.lower() for ch in raw.ch_names]
    
    # Select only channels that we want to use and are available
    channels_to_use = channels_to_use_ref
    # print("channels to use:", channels_to_use)
    channels_present = [ch for ch in channels_to_use if ch.lower() in available_channels]
    ...
    if len(channels_present) < len(channels_to_use) * 1.0:  # 100% threshold
    ...
    try:
        # print("channels present:", channels_present)
        logger.debug(f"Picking channels: {[ch.lower() for ch in channels_present]}")
        raw.pick([ch.lower() for ch in channels_present])
...
def apply_filters(raw, notch_freq=60, bandpass_freqs=(0.5, 100)):
    ...
    raw.filter(l_freq=bandpass_freqs[0], h_freq=bandpass_freqs[1], picks='eeg', njobs=1, verbose=False)
...
def save_processed_data(data, file_path, output_dir, output_format):
    ...
    output_name = base_name.replace('.mefd', f'_preprocessed.{output_format}')
...
def preprocess_eeg(file_path, args, channels_to_use_ref=None, channels_to_use_le=None):
    ...
    # Step 1: Read the MEF file
    raw = read_mef_file(file_path)
    if raw is None:
        return None
    ...
    channel_lists = get_standard_channel_lists()
    channels_to_use_ref = channel_lists['mayo_reduced']
    raw = select_channels(raw, channels_to_use_ref)
    if raw is None:
        return None
...
def find_mef_files(mef_dir, index_date, split_type):
    df = pd.read_csv(f"SSLProbe_{split_type}_index_{index_date}.csv")
    mrn_datetimes = df['mrn_datetime'].values.tolist()
    mef_paths = [f"{mef_dir}/{x}.mefd" for x in mrn_datetimes]
    return mef_paths


def main():
    parser = argparse.ArgumentParser(description='Preprocess MAYO MEF EEG dataset')
    # parser.add_argument('--data-dir', type=str, required=True,
    #                     help='Directory with the MAYO MEF EEG dataset')
    ...
    parser.add_argument('--bandpass-freqs', type=int, nargs=2, default=(0.5, 35),
                        help='Frequency range for bandpass filter (default: 0.5-35 Hz)')
    ...
    '''
    PROCESS ONLY THE SESSIONS LISTED IN INDEX.CSV
    '''
    mef_dir = '/mnt/eplab/Projects/Yoga_EEG_AI/all_routine_mef_4_13_23'
    index_date = '5_12_25'
    split_type = 'val'
    edf_files = find_mef_files(mef_dir, index_date, split_type)
    ...
    def _yield_args(edf_files, args):
